...
So, I tried the gamma distribution and the binomial one (but I would go with the binomial). 
For the link functions for each family, see `help(family)`:

- the Gamma family accepts the links "inverse", "identity" and "log". I will test them all below but I think gamma is not the best choice here anymore.
...
- see also [Difference between logit and probit models](https://stats.stackexchange.com/questions/20523/difference-between-logit-and-probit-models) - "there exists hardly any difference between the results of the two".
...
### Gamma(link = "inverse")
```{r}
m_gamma_inverse <- glmer(E10 ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
...
```
We get the warning `boundary (singular) fit: see ?isSingular`, which is an indication of a degenerate model already (sensu Bates, 2015). The variance `site_f` is zero for example when checking the summary of the model.

### Gamma(link = "identity")
```{r}
m_gamma_identity <- glmer(E10 ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
...
```
We again get the warning `boundary (singular) fit: see ?isSingular` as above.

### Gamma(link = "log")
```{r}
m_gamma_log <- glmer(E10 ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
...
### binomial(link = "logit")
```{r}
m_binomial_logit <- glmer(N1/N0 ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
...
```{r}
try(
  glmer(N1/N0 ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
...
```{r}
m_binomial_probit <- glmer(N1/N0 ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
...
### binomial(link = "log")
```{r}
m_binomial_log <- glmer(N1/N0 ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
...
I think the AIC comparison doesn't make sense here though between distribution families, unless if they are from the same family, but different link functions. I would go anyways with using the `binomial` family with the `weights` argument.
...
cand_set_1[[1]] <- glmer(N1/N0 ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
                         data = baci_dt, weights = N0, family = binomial(link = "logit"))
# Model with interaction in the random effects structure
cand_set_1[[2]] <- glmer(N1/N0 ~ period_ba * treatment_ci + (1|site_f) + (1|year_f) + (1|site_f:year_f), 
                         data = baci_dt, weights = N0, family = binomial(link = "logit"))
# AICc comparison
...
In the case above, according to AICc, we would select the model with the interaction term of the random effects.

However, with the more complex model, `cand_set_1[[2]]`, because we include the interaction term in the random effects structure, `site_f:year_f`, we get the warning message `boundary (singular) fit: see ?isSingular`. If we look at the model structure, we realize that for all the random effects, including their interaction, we have a variance and standard deviation of near zero. This indicates that the maximal model is overparameterized - see Bates (2015) and also Matuschek (2017). I tracked down these papers from this discussion - [What does 'singular fit' mean in Mixed Models?](https://www.researchgate.net/post/What_does_singular_fit_mean_in_Mixed_Models). 
...
We can see already below in the model summary that model 2, with the interaction of random effects (`site_f:year_f`), has a near zero variance for all its random factors, especially for `site_f`:
```{r}
...
The second model, the one with the interaction of random effects (`site_f:year_f`), has a near zero value for the standard deviation of the random effect `site_f`:
```{r}
...
So, according to LRT, if we discard the interaction term from the random effects structure, then we significantly lose information. This confirms also the AICc results. However, the more complex model is "degenerated"/overparameterized according to the diagnostics above, sensu Bates (2015).

All in all, we got contradictory results. AICs & LRT tests favor the more complex model, but that model looks like being "degenerated"/overparameterized. Out of parsimony in such a confusing situation, I will still go with the simpler model. The contradictory situation might indicate that the binomial distribution might be not the best choice and maybe we get more consistent and clear results if we use the beta regression (see betareg R package and details in the section "GLM family" above). I am not sure how to implement that for now and is just a speculation. The analysis below should be valid as well.
...
In our case, the scale parameter for the model without overdispersion control (`model.no.disp.ctrl`) is between 0.75 and 1.4, so accounting for overdispersion is not really justified from that point of view. However, when adjusting for overdispersion, we get a decrease in the overdispersion indicator/parameter. 
...
Both tests go in favor of the more complex model (the model with overdispersion control), however, we again have the same problem as above, that the more complex model becomes "degenerated"/overparameterized when we run the diagnostic proposed in Bates (2015). For `site_f` random effect we get a standard deviation of zero:
```{r}
...
model.no.interaction <- glmer(N1/N0 ~ period_ba + treatment_ci + (1|site_f) + (1|year_f), 
                              data = baci_dt, weights = N0, family = binomial(link = "logit"))
```
...
```{r}
model_no_period <- glmer(N1/N0 ~ treatment_ci + (1|site_f) + (1|year_f), 
                         data = baci_dt, weights = N0, family = binomial(link = "logit"))

anova(model, model_no_period)
```

The AICc comparison also supports the LRT results. The model with period as fixed effect has a smaller AICc with a AICc difference (delta-AICc) > 2 from the model without period:
```{r}
# AICc comparison
...
However, when we run both the LRT test and the parametric bootstrap test (PBtest), we get contradicting results. While for the LRT test below we get the same results as the LRT above (we should get the same Chisq statistic and same corresponding p-value), we don't get anything significant from the PBtest. This is not surprising, because the PBtest is more conservative.
```{r cache=TRUE}
...
However, we see the effect of time/period on the estimated means (model coefficients).
...
Comparing the observed mean values (marked with x symbol in the graph below) with the estimated mean values (model coefficients; marked with a plain dot and connected with lines), we can see that even thought the final GLMM model explained `r R2["delta", "R2c"] %>% round(3)` of the variance in `r varb`, is not that far off. That is, the estimated means for `r varb` are not that far off from the observed mean values.
