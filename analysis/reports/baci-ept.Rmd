...
### Gamma(link = "inverse")
```{r}
m_gamma_inverse <- glmer(ept ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
...
### Gamma(link = "identity")
```{r}
m_gamma_identity <- glmer(ept ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
...
### Gamma(link = "log")
```{r}
m_gamma_log <- glmer(ept ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
...
### binomial(link = "logit")
```{r}
m_binomial_logit <- glmer(N_EPT/N ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
...
```{r}
m_binomial_probit <- glmer(N_EPT/N ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
                           data = baci_dt, weights = N, family = binomial(link = "probit"))
...
```{r}
try(
  m_binomial_log <- glmer(N_EPT/N ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
                          data = baci_dt, weights = N, family = binomial(link = "log"))
)
```
...
cand_set_1[[1]] <- glmer(N_EPT/N ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
                         data = baci_dt, weights = N, family = binomial(link = "logit"))
# Model with interaction in the random effects structure
cand_set_1[[2]] <- glmer(N_EPT/N ~ period_ba * treatment_ci + (1|site_f) + (1|year_f) + (1|site_f:year_f), 
                         data = baci_dt, weights = N, family = binomial(link = "logit"))
# AICc comparison
...
However, for the second model, if we look at the model structure, we realize that for all the random effects, including their interaction, we have a variance and standard deviation of near zero. This indicates that the model is overparameterized - see Bates (2015) and also Matuschek (2017). I tracked down these papers from this discussion - [What does 'singular fit' mean in Mixed Models?](https://www.researchgate.net/post/What_does_singular_fit_mean_in_Mixed_Models). 
...
This is the case for the 2nd model:
```{r}
...
In the LRT below we notice there is a significant difference between the two models. This test, as the AICc one, favors the more complex model (2nd model).
```{r}
# LRT test
...
We could also try the gamma distribution, though that is more suitable when the dependent variable varies between 0 and infinity. In our case it varies between `r baci_dt[[varb]] %>% range %>% round(digits = 2)`.
...
In our case, the scale parameter for the model without overdispersion control (`model.no.disp.ctrl`) is far above 1.4, so accounting for overdispersion can be justified. When adjusting for overdispersion, we get a big drop in the overdispersion indicator/parameter. 
...
Also both the LRT and AICc test go in favor of the model with overdispersion control.

Lucky, both models are NOT "degenerated"/overparameterized when we run the diagnostic proposed in Bates (2015). For both we get standard deviations different from zero for their random effects:
```{r}
summary(lme4::rePCA(model.no.disp.ctrl))
summary(lme4::rePCA(model.with.disp.ctrl))
```

So, we can go with the model accounting for overdispersion.
```{r}
model <- model.with.disp.ctrl
```
...
There seem to be no obvious issues in the distribution of the residuals. Some residuals seem to misbehave. In the plot "fitted vs residuals", the model seems to underestimate small values of `r varb` and also overestimates the big values of `r varb`. However, in the "Fitted values vs Observed", the model seems to perform very well.
...
scatter.smooth(fitted(model), baci_dt[[varb]],
               main = "Fitted values vs Observed",
...
model.no.interaction <- glmer(N_EPT/N ~ period_ba + treatment_ci + (1|site_f) + (1|year_f) + (1|site_f:year_f) + (1|obs), 
                              data = baci_dt, weights = N, family = binomial(link = "logit"))
```
...
Also the AICc test confirms there is no difference between the two models:
```{r}
aictab(cand.set = list(model, model.no.interaction), 
       modnames = c("1-with-baci",
                    "2-without-baci"), 
       second.ord = TRUE)
```
...
Comparing the observed mean values (marked with x symbol in the graph below) with the estimated mean values (model coefficients; marked with a plain dot and connected with lines), we can see that the model fits good enough. That is, the estimated means for `r varb` are close to the observed mean values.
