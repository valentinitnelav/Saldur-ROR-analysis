---
title: "BACI analysis for Shannon evenness (Hill’s ratio, E10)"
...
As we saw in the graphs above, the Shannon evenness (Hill’s ratio, E10) is a continuous variable with values between 0 and 1. Since negative values are not allowed, we can use the gamma error distribution.
...
```{r}
glmer(E10 ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
      data = baci_dt, family = Gamma(link = "log"))
```

We also tried the "Nelder_Mead" optimizing function, which failed as well:
```{r}
glmer(E10 ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
      data = baci_dt, family = Gamma(link = "log"),
...
Finally, we tried the `L-BFGS-B` method from the package `optimx` and this solved the convergence issue (see [suggestion](https://stats.stackexchange.com/a/243225/95505)).
...
models[[1]] <- glmer(E10 ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
                     data = baci_dt, family = Gamma(link = "log"),
                     control = glmerControl(optimizer = "optimx",
                                            optCtrl = list(method = "L-BFGS-B")))

# Model with interaction in the random effects structure
models[[2]] <- glmer(E10 ~ period_ba * treatment_ci + (1|site_f) + (1|year_f) + (1|site_f:year_f), 
                     data = baci_dt, family = Gamma(link = "log"),
                     control = glmerControl(optimizer = "optimx",
                                            optCtrl = list(method = "L-BFGS-B")))
```

For both models, we get the warning message `boundary (singular) fit: see ?isSingular`. If we look at the model structure, we observe that for the random effect `site_f` we have a variance and standard deviation of zero. This indicates that the models might be overparameterized - see Bates (2015) and also Matuschek (2017).
```{r}
summary(models[[1]])
summary(models[[2]])
```

Bates (2015) also proposed the helper function `rePCA` to carry PCA of random-effects covariance matrix. If there are zero or near zero values in the output matrices, then those models indicate overfitting and are overparameterized.
```{r}
summary(lme4::rePCA(models[[1]]))
summary(lme4::rePCA(models[[2]]))
```

It is interesting to observe that the random effect `site_f` has a null variance in both models. However, in the more complex model, the interaction between the two random factors is different from zero. Fitting a model without `site_f` solves the overfitting warning.

```{r}
models[[3]] <- glmer(E10 ~ period_ba * treatment_ci + (1|year_f) + (1|site_f:year_f), 
                     data = baci_dt, family = Gamma(link = "log"),
                     control = glmerControl(optimizer = "optimx",
                                            optCtrl = list(method = "L-BFGS-B")))
summary(lme4::rePCA(models[[3]]))
```
...
```{r}
# AICc comparison
AIC.res.table <- aictab(cand.set = models, 
...
In our case, the scale parameter for the model without overdispersion control (`model.no.disp.ctrl`) is smaller than 0.75, so accounting for overdispersion can be justified from that point of view. However, when adjusting for overdispersion, the model fitting fails. Attempts with other models also failed or runs into convergence issues.

```{r}
try( update(models[[1]], . ~ . + (1|obs)) )
try( update(models[[2]], . ~ . + (1|obs)) )
```

Out of parsimony, we select for further use the best performing model in terms of AIC without overdispersion control.
```{r}
...
model.no.interaction <- glmer(E10 ~ period_ba + treatment_ci + (1|year_f) + (1|site_f:year_f), 
                              data = baci_dt, family = Gamma(link = "log"),
                              control = glmerControl(optimizer = "optimx",
                                                     optCtrl = list(method = "L-BFGS-B")))
```
