...
We could not use the beta distribution for the errors because EPT% is not satisfying the interval 0 < x < 1, as it contains values of 1. Instead, we can consider the variable as a form of success rate from total cases, `N_EPT/N`, where N_EPT and N are integers. In that case we can use the binomial distribution.
...
The AIC test favors that the more complex model (with `site_f:year_f`). The LRT test indicates that there are significant differences between the two models. We pick the more complex model for further analysis.
...
The BACI effect estimated from the model, which is the difference of the two changes (control after − control before) − (impact after − impact before]), was `r round(baci, 3)` ± `r round(baci_ci$SE, 3)` standard error.
