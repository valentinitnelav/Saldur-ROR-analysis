---
title: "BACI analysis for % Ephemeroptera–Plecoptera–Tricoptera (% EPT)"
...
As we saw in the graphs above, the % Ephemeroptera–Plecoptera–Tricoptera (% EPT) is a continuous variable with values between 0 and 1. Since negative values are not allowed, we can use the gamma error distribution.
...
models[[1]] <- glmer(ept ~ period_ba * treatment_ci + (1|site_f) + (1|year_f), 
                     data = baci_dt, family = Gamma(link = "log"))

# Model with interaction in the random effects structure
models[[2]] <- glmer(ept ~ period_ba * treatment_ci + (1|site_f) + (1|year_f) + (1|site_f:year_f), 
                     data = baci_dt, family = Gamma(link = "log"))
```
...
There are not major differences between the two models in terms of AIC.

Additionally to the AIC test, we can also use the log likelihood ratio test (LRT) as per Bates (2015). 

The main idea is that if the LRT test gives a significant p-value, then that indicates that there are statistically significant differences between the two nested models. That is, we can lose information if we go with the simpler model. In such cases, we can keep the more complex model, which can capture better the variance in the given data. However, if there are no significant differences between the models, then we can safely discard the more complex model.
...
The LRT results below indicate no statistically significant differences between the two nested models. Out of parsimony we can keep the simpler model for further use.
```{r}
# LRT test
...
In our case, the scale parameter for the model without overdispersion control (`model.no.disp.ctrl`) is smaller than 0.75, so accounting for overdispersion can be justified from that point of view. However, when adjusting for overdispersion, the model becomes overparameterized and is overfitting.
...
We can observe below the overfitting nature of the model with overdispersion control.
```{r}
# fitted vs observed
scatter.smooth(fitted(model.with.disp.ctrl), baci_dt[[varb]],
               xlab = "Fitted Values",
               ylab = "Observed")
abline(0, 1, col = "red")
```

Due to the overfitting issue and out of parsimony, we select for further use the model without overdispersion control.
```{r}
...
model.no.interaction <- glmer(ept ~ period_ba + treatment_ci + (1|year_f) + (1|site_f:year_f), 
                              data = baci_dt, family = Gamma(link = "log"))
```
